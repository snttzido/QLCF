...
Full Python: Flask + sqlite3 (thư viện chuẩn) + Jinja2 + Bootstrap 5 (CDN)
...
app = Flask(__name__)
app.secret_key = 'cafe-quan-ly-secret-key-2026'  # đổi khi triển khai thật
...
def staff_menu_add():
    if request.method == 'POST':
        ...
        if not ten or gia < 0:
            flash('Vui lòng nhập tên món và giá hợp lệ.', 'error')
        else:
            db.execute(
...
def staff_menu_toggle(mon_id):
    mon = db.query('SELECT * FROM mon_an WHERE id = ?', (mon_id,), one=True)
    if mon:
        db.execute('UPDATE mon_an SET con_ban=? WHERE id=?', (0 if mon['con_ban'] else 1, mon_id))
        flash('Đã cập nhật trạng thái món.', 'success')
    return redirect(url_for('staff_menu'))


@app.route('/staff/menu/delete/<int:mon_id>', methods=['POST'])
def staff_menu_delete(mon_id):
    db.execute('DELETE FROM mon_an WHERE id = ?', (mon_id,))
...
def staff_payment():
    don_list = db.query('''SELECT don_hang.*, ban.so_ban FROM don_hang
                            LEFT JOIN ban ON don_hang.ban_id = ban.id
                            WHERE don_hang.trang_thai != 'Đã hủy' AND don_hang.da_thanh_toan = 0
